...
module.exports = function (__runtime__, scope) {
  if (typeof scope.RunningQueueDispatcher === 'undefined') {
    ...
    let Timers = require('./Timers.js')(__runtime__, scope)
    let lockableStorages = require('./LockableStorage.js')(__runtime__, scope)

    let pwd = files.cwd()
    let logLibExists = files.exists(pwd + '/lib/LogUtils.js')
    let _logUtils = logLibExists ? require(pwd + '/lib/LogUtils.js')(__runtime__, scope) : {
      logInfo: (str) => {
        console.log(str)
      },
      warnInfo: (str) => {
    ...
    if (logLibExists) {
      _logUtils.debugForDev('日志工具类存在')
    } else {
      _logUtils.debugForDev('日志工具类不存在, 当前目录：' + pwd)
    }
    ...
    function RunningQueueDispatcher () {
      ...
      this.getStorage = function () {
        return lockableStorages.create(STORAGE_KEY)
      }
        ...
        if (storedLockStr) {
          ...
          if (storedLock.source === taskInfo.source) {
            if (storedLock.engineId === taskInfo.engineId) {
              ...
              if (runningEngines && runningEngines.find((engine) => engine.id === storedLock.engineId)) {
                return false
              } else {
      ...
      this.removeRunningTask = function (checkOwner) {
        ...
        if (runningTask !== null) {
          ...
          if (runningTask.source === taskInfo.source && (!checkOwner || runningTask.engineId === taskInfo.engineId)) {
            _logUtils.debugInfo('移除' + (checkOwner ? '重复运行误创建的' : '') + '运行中任务')
            // 不加锁直接移除
            this.getStorage().put(RUNNING_KEY, '')
      ...
      this.addRunningTask = function () {
        ...
        if (runningTask !== null) {
          ...
          if (runningTask.source === taskInfo.source) {
            _logUtils.debugInfo('运行中脚本任务和当前任务相同，直接继续')
          ...
          if (waitingTask !== null) {
            ...
            if (waitingTask.source === taskInfo.source) {
              ...
              if (this.lock(3)) {
                ...
                _logUtils.errorInfo('获取锁失败，无法执行等待中任务，当前任务也未成功入队列：' + JSON.stringify(taskInfo))
                this.setUpAutoStart(taskInfo.source, 10)
                exit()
              }
            }
    ...
    scope.RunningQueueDispatcher = new RunningQueueDispatcher()
  }
  return scope.RunningQueueDispatcher
