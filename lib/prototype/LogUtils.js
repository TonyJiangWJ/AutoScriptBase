...
function Logger () {
  ...
  this.showCostingInfo = () => {
    console.verbose('日志入队总耗时：' + this.enqueueCostCounter + 'ms 写入文件总耗时：' + this.fileWriteCostCounter + 'ms 备份日志文件总耗时：' + this.backupCostCounter + 'ms')
  }
}
...
function AsyncLogger () {
  ...
  this.executeThreadPool = new ThreadPoolExecutor(1, 1, 60, TimeUnit.SECONDS, new LinkedBlockingQueue(10))
  this.writingList = []
  this.readingList = []
    ...
    while (true && _config.save_log_file && (_config.isRunning || self.readingList.length !== 0 || self.writingList.length !== 0)) {
      ...
      try {
        ...
        if (self.readingList.length === 0) {
          console.warn('脚本可能已终止执行')
          if (self.writingList.length !== 0) {
            // 切换缓冲区，将缓冲区内容全部写入日志
            console.verbose('切换缓冲区，将缓冲区内容全部写入日志')
        ...
        if (loggerRunning) {
          ...
          console.verbose('写入日志到文件耗时：' + cost + 'ms')
        }
        // 置空
        self.readingList = []
...
if (_config.async_save_log_file) {
  console.verbose('使用异步日志')
} else {
  console.verbose('使用同步日志')
}
/**
