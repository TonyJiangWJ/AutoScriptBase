...
function CommonFunctions () {
  ...
  this.setUpAutoStart = function (minutes) {
    if (_config.not_setup_auto_start && _config.disable_all_auto_start) {
      return
    }
    if (minutes <= 0) {
  ...
  */
  ...
  this.delayStartIfInPayCode = function () {
    ...
    if (this._widgetUtils.widgetGetOne(_config.delay_start_pay_code_content || '向商家付(钱|款)', 1000, false, true)) {
      _logUtils.warnInfo('当前在支付码界面，延迟5分钟后执行', true)
      this.setUpAutoStart(5)
      _config.forceStop = true
      exit()
    }
  ...
  this.convertImageFromSingleChannel = function (singleChannelImage) {
    ...
    return com.stardust.autojs.core.image.ImageWrapper.ofBitmap(singleChannelImage.getBitmap())
  }
}
